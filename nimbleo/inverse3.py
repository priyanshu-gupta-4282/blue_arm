...
# === Main Program ===
try:
    home_position = [0, 110, 180, 180, 180]  # Base, Shoulder, Elbow, Wrist, Fingers angles in degrees
    drop_position = [90, 70, 120, 70, ]  # Example position including fingers angle

    # Step 1: Calibrate the stepper motor
    calibrate_stepper()

    # Step 2: Move all servos to 90 degrees immediately after the program starts
    print("Moving all servos to 90 degrees...")
    for i in range(len(servo_pins)):
        set_servo_angle_slow(servo_pins[i], servo_current_angles[i], 90)  # Move to 90 degrees
        servo_current_angles[i] = 90  # Update current angles

    # Step 3: Return servos to home position
    ...
    # Step 4: Get user input for the end effector position (x, y, z)
    ...
    # Step 5: Move to the calculated position in the order: Base, Finger, Wrist, Elbow, Shoulder
   ...
   # Shoulder (servo 0)
    ...
    # Step 6: Move to drop position (for example)
    ...
    # Cleanup all GPIO and PWM resources
    pi.stop()
    GPIO.cleanup()
